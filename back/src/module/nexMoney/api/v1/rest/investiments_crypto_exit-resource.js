...
const investimentsCryptoExit = model.investimentsCryptoExit;

// Listar todos os investimentos de renda variável (cripto)
router.get("/api/v1/rest/investiments-crypto-exit", async (req, res) => {
  try {
    const data = await investimentsCryptoExit.findAll();
    res.json(data);
  } catch (error) {
    console.error(error);
...
router.get("/api/v1/rest/investiments-crypto-exit/:id", async (req, res) => {
  try {
    const data = await investimentsCryptoExit.findByPk(req.params.id);
...
router.post("/api/v1/rest/investiments-crypto-exit", async (req, res) => {
  try {
    const {
      typeInvestment,
      name,
    ...
    const created = await investimentsCryptoExit.create({
      typeInvestment,
      name,
      initialValue,
      initialAmount,
...
router.put("/api/v1/rest/investiments-crypto-exit/:id", async (req, res) => {
  try {
    const data = await investimentsCryptoExit.findByPk(req.params.id);
    if (!data) return res.status(404).json({ error: "não encontrado" });
...
router.delete("/api/v1/rest/investiments-crypto-exit/:id", async (req, res) => {
  try {
    const data = await investimentsCryptoExit.findByPk(req.params.id);
    if (!data) return res.status(404).json({ error: "não encontrado" });
    await data.destroy();
